...
func ServiceRegister(Router *gin.RouterGroup) {
	...
	Router.GET("/service_delete", Service.ServiceDelete)
	Router.GET("/service_detail", Service.ServiceDetail)
	Router.GET("/service_stat", Service.ServiceStat)
	Router.POST("/service_add_http", Service.ServiceAddHTTP)
	Router.POST("/service_update_http", Service.ServiceUpdateHTTP)

	Router.POST("/service_add_tcp", Service.ServiceAddTCP)
	Router.POST("/service_update_tcp", Service.ServiceUpdateTcp)
	Router.POST("/service_add_grpc", Service.ServiceAddGrpc)
	Router.POST("/service_update_grpc", Service.ServiceUpdateGrpc)
}
...
// @Accept  json
// @Produce  json
// @Param info query string false "服务ID"
...
func (service *ServiceController) ServiceAddHTTP(c *gin.Context) {
	...
	if _, err = httpUrl.Find(c, tx, httpUrl); err == nil {
		tx.Rollback()
		middleware.ResponseError(c, 2004, errors.New("域名或前缀已存在"))
		return
	}
	...
	accessControl := &dao.AccessControl{
		...
		WhiteHostName:     params.WeightList,
...
func (admin *ServiceController) ServiceAddTCP(c *gin.Context) {
	...
	if _, err := infoSearch.Find(c, lib.GORMDefaultPool, infoSearch); err != nil {
		middleware.ResponseError(c, 2001, errors.New("服务已被占用,请重新输入"))
		return
	}
	...
	if _, err := tcpRuleSearch.Find(c, lib.GORMDefaultPool, tcpRuleSearch); err != nil {
		middleware.ResponseError(c, 2002, errors.New("服务端口已被占用,请重新输入"))
		return
	}
