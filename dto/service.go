...
type ServiceListInput struct {
	Info     string `json:"info" form:"info" comment:"关键词" example:"" validata:"required"`       //关键词
	PageNo   int    `json:"page_no" form:"page_no" comment:"页数" example:"1" validata:"required"` //页数
	PageSize int    `json:"page_size" form:"page_size" comment:"每页长度" example:"20" validata:""`  //每页长度
}

type ServiceDeleteInput struct {
	ID int64 `json:"id" form:"id" comment:"服务ID" example:"56" validata:"required"` //服务ID
}

type ServiceAddHTTPInput struct {
	ServiceName string `json:"service_name" form:"service_name" comment:"服务名称" example:"" validata:"required,valid_service_name"` //服务名称
	ServiceDesc string `json:"service_desc" form:"service_desc" comment:"服务描述" example:"" validata:"required,max=255,min=1"`      //服务描述

	RuleType       int    `json:"rule_type" form:"rule_type" comment:"接入类型" example:"" validata:"max=1,min=0"`                           //接入类型
	Rule           string `json:"rule" form:"rule" comment:"服务路径" example:"" validata:"required,valid_rule"`                             //服务路径
	NeedHttps      int    `json:"need_https" form:"need_https" comment:"支持http" example:"" validata:"max=1,min=0"`                       //支持http
	NeedStripUri   int    `json:"need_srtip_uri" form:"need_srtip_uri" comment:"支持srtip_uri" example:"" validata:"max=1,min=0"`          //支持srtip_uri
	NeedWebsocket  int    `json:"need_websocket" form:"need_websocket" comment:"支持websocket" example:"" validata:"max=1,min=0"`          //支持websocket
	UrlRewrite     string `json:"url_rewrite" form:"url_rewrite" comment:"url重写" example:"" validata:"valid_url_rewrite"`                //url重写
	HeaderTransfor string `json:"header_transfor" form:"header_transfor" comment:"header转换" example:"" validata:"valid_header_transfor"` //header转换

	OpenAuth          int    `json:"open_auth" form:"open_auth" comment:"是否开启权限 1=开启" example:"" validata:"max=1,min=0"`    //是否开启权限 1=开启
	BlackList         string `json:"black_list" form:"black_list" comment:"黑名单" example:"" validata:""`                     //黑名单
	WhiteList         string `json:"white_list" form:"white_list" comment:"白名单" example:"" validata:""`                     //白名单
	ServiceFlowLimit  int    `json:"service_flow_limit" form:"service_flow_limit" comment:"服务端限流" example:"" validata:""`   //服务端限流
	ClientipFlowLimit int    `json:"clientip_flow_limit" form:"clientip_flow_limit" comment:"客户端限流" example:"" validata:""` //客户端限流

	RoundType              int    `json:"round_type" form:"round_type" comment:"轮询方式" example:"" validata:"max=3,min=0"`                                //轮询方式
	IpList                 string `json:"ip_list" form:"ip_list" comment:"ip列表" example:"" validata:"required,valid_ip_list"`                           //ip列表
	WeightList             string `json:"weight_list" form:"weight_list" comment:"权重列表" example:"" validata:"required,valid_weight_list"`               //权重列表
	UpstreamConnectTimeout int    `json:"upstream_connect_timeout" form:"upstream_connect_timeout" comment:"建立连接超时, 单位s" example:"" validata:"min=0"`   //建立连接超时, 单位s
	UpstreamHeaderTimeout  int    `json:"upstream_header_timeout" form:"upstream_header_timeout" comment:"获取header超时, 单位s" example:"" validata:"min=0"` //获取header超时, 单位s
	UpstreamIdleTimeout    int    `json:"upstream_idle_timeout" form:"upstream_idle_timeout" comment:"链接最大空闲时间, 单位s" example:"" validata:"min=0"`       //链接最大空闲时间, 单位s
	UpstreamMaxIdle        int    `json:"upstream_max_idle" form:"upstream_max_idle" comment:"最大空闲链接数" example:"" validata:"min=0"`                     //最大空闲链接数
}
...
type ServiceUpdateHTTPInput struct {
	...
	Rule           string `json:"rule" form:"rule" comment:"接入路径：域名或者前缀" example:"/test_http_service_indb" validate:""`                  //域名或者前缀
...
type ServiceUpdateTcpInput struct {
	ID                int64  `json:"id" form:"id" comment:"服务ID" validate:""`
	ServiceName       string `json:"service_name" form:"service_name" comment:"服务名称" validate:""`
	ServiceDesc       string `json:"service_desc" form:"service_desc" comment:"服务描述" validate:""`
	Port              int    `json:"port" form:"port" comment:"端口，需要设置8001-8999范围内" validate:"required,min=8001,max=8999"`
	OpenAuth          int    `json:"open_auth" form:"open_auth" comment:"是否开启权限验证" validate:""`
	BlackList         string `json:"black_list" form:"black_list" comment:"黑名单IP，以逗号间隔，白名单优先级高于黑名单" validate:"valid_iplist"`
	WhiteList         string `json:"white_list" form:"white_list" comment:"白名单IP，以逗号间隔，白名单优先级高于黑名单" validate:"valid_iplist"`
...
type ServiceListItemOutput struct {
	...
	QPD         int64  `json:"qpd" form:"qpd" `                   //QPS
...
type ServiceListOutput struct {
	Total int64                   `json:"total" form:"total" comment:"总页数" example:"" validata:""` //总页数
	List  []ServiceListItemOutput `json:"list" form:"list" comment:"页数" example:"" validata:""`    //列表
}
